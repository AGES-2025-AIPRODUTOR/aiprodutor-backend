...
export class HarvestsController {
  ...
  @ApiResponse({
    status: 409,
    description: 'Já existe uma safra com este nome.',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Dados do painel retornados.',
    type: HarvestPanelResponseDto,
  })
  @ApiResponse({ status: 404, description: 'Safra não encontrada.' })
  getHarvestPanel(
    @Param('id', ParseIntPipe) id: number,
  ): Promise<HarvestPanelResponseDto> {
  ...
  @ApiParam({
    name: 'id',
    description: 'ID do Produtor para o qual o histórico será buscado',
  })
  @ApiResponse({
    status: 200,
    description: 'Histórico de safras retornado com sucesso.',
    type: [HarvestHistoryResponseDto],
  })
  ...
  @ApiOperation({
    summary: 'Lista todas as safras em andamento de um produtor',
  })
  @ApiParam({ name: 'producerId', description: 'ID do produtor' })
  @ApiResponse({
    status: 200,
