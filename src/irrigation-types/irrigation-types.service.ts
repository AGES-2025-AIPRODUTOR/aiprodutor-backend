...
export class IrrigationTypesService {
  ...
  async update(
    id: number,
    updateIrrigationTypeDto: UpdateIrrigationTypeDto,
  ): Promise<IrrigationTypes> {
  ...
  async remove(id: number) {
    ...
    const irrigationTypeInUse =
      await this.areasRepository.existsByIrrigationTypeId(id);
    if (irrigationTypeInUse) {
      throw new BadRequestException(
        'Não é possível remover! tipo de irrigação está em uso por uma ou mais áreas.',
      );
    }
